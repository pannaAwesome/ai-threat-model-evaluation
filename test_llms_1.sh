#!/bin/bash
#SBATCH --job-name=llm_test_job
#SBATCH --output=llm_output_%x_%j.txt
#SBATCH --error=llm_error_%x_%j.txt
...
MODELS=("yi:9b-v1.5-q4_K_M" "yi:9b-chat-v1.5-q4_K_M")
MAX_RETRIES=3
...
for model in "${MODELS[@]}"; do
    ...
    bash test_run_one_experiment.sh "$TOOL_NAME" "$APPLICATION" "$model" 5
done
